...
After this introductory code, check if the Submit button has been clicked:
mysql_select_db($dbdatabase, $db);
if($_POST['submit']) {
...
. "', 0);";
mysql_query($sql);
Construct the email to send to the user for verification:
mysql_query($sql);
$mail_body=<<<_MAIL_
Hi $validusername,
...
echo "A link has been emailed
to the address you entered below. Please follow the
link in the email to validate your account.";
}
}
else {
header("Location: " . $config_basedir .
...
?>
<h2>Register</h2>
To register on the <?php echo $config_forumsname; ?> forums, fill in
the form below.
